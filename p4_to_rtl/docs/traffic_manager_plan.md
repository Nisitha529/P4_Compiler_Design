...
| 3 | N queues + round-robin scheduler, queue from `egress_port` | reordering — the real risk | new TM harness; per-queue order preserved, cross-queue interleaved |
| 4 | `enq_qdepth`/`deq_qdepth`, tail drop; port `ecn` | qdepth semantics under congestion | `ecn_p4rtl` marks under real queue build-up |
| 5 | `mcast_group` replication, copy-counted slot release | replication, slot lifetime | `multicast_p4rtl`; loop-prune via `ingress_port` |
