module regprobe_top #(
  ...
  logic [0:0] slot_meta_v1 [0:NSLOT-1];
  logic [31:0] slot_meta_pos [0:NSLOT-1];
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (proc_out_valid) begin
        ...
        slot_meta_v1[cmp_slot] <= proc_out_meta_v1;
        slot_meta_pos[cmp_slot] <= proc_out_meta_pos;
      end
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (emit_hdr) begin
        ...
        out_meta_v1 <= slot_meta_v1[tx_slot];
        out_meta_pos <= slot_meta_pos[tx_slot];
