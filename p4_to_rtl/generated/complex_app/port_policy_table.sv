module port_policy_table #(
  ...
  output logic [8:0] p_egress_port,

  // Control-plane write port (synchronous)
  input  logic        cp_wr_en,
  input  logic [5:0] cp_wr_idx,  // unused: exact-match tables self-address via hash(key)
  input  logic [8:0] cp_wr_key_ingress_port,
  input  logic [0:0] cp_wr_action,
  input  logic [8:0] cp_wr_p_egress_port
);
  ...
  logic [8:0] mem_p_egress_port[0:DEPTH-1];
  ...
  // XOR-fold hash: 9-bit key -> 6-bit BRAM address
  function automatic logic [5:0] hash_key(input logic [11:0] k);
    logic [5:0] h;
    integer c;
    begin
      h = '0;
  ...
  assign wr_key_concat = {3'd0, cp_wr_key_ingress_port};
  logic [5:0] wr_addr;
  assign wr_addr = hash_key(wr_key_concat);

  logic [11:0] lkp_key_concat;
  assign lkp_key_concat = {3'd0, lkp_ingress_port};
  logic [5:0] lkp_addr;
  ...
  always_ff @(posedge clk) begin
    if (cp_wr_en) begin
      ...
      mem_p_egress_port[wr_addr] <= cp_wr_p_egress_port;
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      p_r_egress_port <= mem_p_egress_port[lkp_addr];
    end
  end

  logic hit_c; assign hit_c = valid_r && (mem_key_r_ingress_port == key_r_ingress_port);
  logic [0:0] action_id_c;
  assign action_id_c = hit_c ? action_id_r : 1'd0;
  logic [8:0] p_egress_port_c;
  assign p_egress_port_c = hit_c ? p_r_egress_port : 9'b0;
