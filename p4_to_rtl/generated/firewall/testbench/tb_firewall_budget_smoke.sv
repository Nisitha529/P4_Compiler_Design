// ============================================================================
// tb_firewall_budget_smoke.sv -- opt-in table-tree-splitting cross-file
// invariant check (Phase 5, architecture-redesign effort). NOT a
// replacement for tb_firewall.sv's cycle-exact functional regression
// suite, and does not edit it. tb_ipv4_lpm_table_standalone.sv already
...
// emit_processing.py's table_noop_stages count for the same table -- a
// drift there wouldn't crash, it would silently read hit/action_id one
// cycle too early. Deliberately does NOT assert an exact cycle count
// (that count is supposed to grow with --target-freq-mhz): it polls for
// valid_out within a generous, budget-independent bound and checks the
// result once seen.
//
// Only exercises the ipv4_lpm path (hit->forward, miss->drop) -- leaves
// check_ports unconfigured so it always misses, which short-circuits
// firewall.p4's bloom-filter logic entirely (see apply block: it's
// nested inside `if (check_ports.apply().hit)`), keeping this test
// focused on exactly the code path the table-tree split touches.
//
// Compile (from this directory, against sources generated with
// `python3 main.py firewall --target-freq-mhz 400 --device artix7`):
//   iverilog -g2012 -o sim tb_firewall_budget_smoke.sv \
//     ../processing_generated.sv ../ipv4_lpm_table.sv ../check_ports_table.sv
//   vvp sim
// ============================================================================
`timescale 1ns/1ps

module tb_firewall_budget_smoke;
  ...
  // Comfortably above the worst case this compiler would realistically be
  // asked to hit (see tb_ipv4_lpm_table_standalone.sv's WAIT_CYCLES note).
  ...
  // Pulses valid_in for one cycle, then polls valid_out for up to
  // MAX_WAIT_CYCLES edges -- no hardcoded latency, unlike tb_firewall.sv's
  // send_packet (deliberately: that latency is exactly what this feature
  // changes).
  ...
  initial begin
    ...
    $display("== firewall processing_generated budget-smoke check (opt-in table-tree split) ==");

    // Entry: 192.168.0.0/24 -> ipv4_forward, dstAddr=AABBCCDDEEFF, port=5
    lpm_write(0, 32'hC0A80000, 6'd24, 2'd1, 48'hAABBCCDDEEFF, 9'd5);

    // T1: LPM hit -> forward, not dropped, dstAddr rewritten, TTL-1.
    pr_ipv4_dst = 32'hC0A80005; // 192.168.0.5, inside the /24
    send_and_wait(got_valid);
    chk("T1: valid_out seen within bound", got_valid);
    if (got_valid) begin
      chk("T1: not dropped", pr_drop === 1'b0);
      chk("T1: dstAddr rewritten to entry's MAC", pr_o_eth_dst === 48'hAABBCCDDEEFF);
      chk("T1: ttl decremented", pr_o_ipv4_ttl === 8'd63);
      chk("T1: egress_spec == 5", pr_o_egress_spec === 9'd5);
    end

    // T2: LPM miss -> default_action=drop().
    pr_ipv4_dst = 32'h08080808; // 8.8.8.8, no matching entry
    send_and_wait(got_valid);
    chk("T2: valid_out seen within bound", got_valid);
    if (got_valid) begin
      chk("T2: dropped on miss", pr_drop === 1'b1);
    end
