module ueprobe_top #(
  ...
  wire proc_drop;
  logic iss_fire;
  wire [15:0] proc_out_meta_res;
  wire [7:0] proc_out_meta_res2;
  ...
  processing_generated u_proc (
    ...
    .out_meta_res  (proc_out_meta_res),
    .out_meta_res2  (proc_out_meta_res2),
  ...
  logic [15:0] slot_meta_res [0:NSLOT-1];
  logic [7:0] slot_meta_res2 [0:NSLOT-1];
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (proc_out_valid) begin
        ...
        slot_meta_res[cmp_slot] <= proc_out_meta_res;
        slot_meta_res2[cmp_slot] <= proc_out_meta_res2;
      end
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (emit_hdr) begin
        ...
        out_meta_res <= slot_meta_res[tx_slot];
        out_meta_res2 <= slot_meta_res2[tx_slot];
