...
module tb_load_balance;
  ...
  initial begin
    ...
    pr_ipv4_ttl = 8'd64; pr_ipv4_valid = 1; @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk); #1;
    ...
    pr_ipv4_ttl = 8'd0; @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk); #1;
    ...
    pr_ipv4_valid = 0; @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk); #1;
    chk("PR4: hit=0",                      !pr_hit_out);
    chk("PR4: eth_dst unchanged",          pr_o_eth_dst == 48'hAABBCCDDEEFF);
    pr_ipv4_valid = 1;

    // ── PR5: valid_out pipeline register ──────────────────────────────────────
    // ecmp_group and ecmp_nhop are each now a 2-cycle boundary (lookup +
    // output register) -> valid_out is 5 cycles behind valid_in
    // (1 baseline + 4 boundary).
    $display("  PR5: valid_out is five-cycle registered (1 baseline + 4 boundary)");
    do_reset();
    pr_valid_in = 1; #1;
    ...
    pr_valid_in = 0;
    @(posedge clk); #1;
    chk("PR5: valid_out=0 after 2nd posedge", !pr_valid_out);
    @(posedge clk); #1;
    chk("PR5: valid_out=0 after 3rd posedge", !pr_valid_out);
    @(posedge clk); #1;
    chk("PR5: valid_out=0 after 4th posedge", !pr_valid_out);
    @(posedge clk); #1;
    chk("PR5: valid_out=1 after 5th posedge",  pr_valid_out);
    @(posedge clk); #1;
    ...
    pr_meta_ecmp = 14'd0; @(posedge clk); @(posedge clk); @(posedge clk); @(posedge clk); #1;
