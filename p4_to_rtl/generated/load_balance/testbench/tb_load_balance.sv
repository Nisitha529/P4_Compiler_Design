...
module tb_load_balance;
  ...
  initial begin
    ...
    pr_ipv4_ttl = 8'd64; pr_ipv4_valid = 1; @(posedge clk); @(posedge clk); #1;
    ...
    pr_ipv4_ttl = 8'd0; @(posedge clk); @(posedge clk); #1;
    ...
    pr_ipv4_valid = 0; @(posedge clk); @(posedge clk); #1;
    chk("PR4: hit=0",                      !pr_hit_out);
    chk("PR4: eth_dst unchanged",          pr_o_eth_dst == 48'hAABBCCDDEEFF);
    pr_ipv4_valid = 1;

    // ── PR5: valid_out pipeline register ──────────────────────────────────────
    // load_balance now has two registered boundaries in sequence:
    // ecmp_group's pipelined priority tree, then ecmp_nhop's exact-match
    // hash+BRAM lookup -> valid_out is 3 cycles behind valid_in
    // (1 baseline + 2 boundaries).
    $display("  PR5: valid_out is three-cycle registered (1 baseline + 2 boundaries)");
    do_reset();
    pr_valid_in = 1; #1;
    ...
    @(posedge clk); #1;
    chk("PR5: valid_out=1 after 3rd posedge",  pr_valid_out);
    @(posedge clk); #1;
    ...
    pr_meta_ecmp = 14'd0; @(posedge clk); @(posedge clk); #1;
