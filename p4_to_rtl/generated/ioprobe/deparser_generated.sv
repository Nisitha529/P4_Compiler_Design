module deparser_generated (
  ...
  // Packed output [271:0]  layout (MSB first): eth(112b) | ipv4(160b)
  ...
  always_comb begin
    ...
    if (eth_valid) begin
      pkt_hdr_out[271:160] = {eth_dst, eth_src, eth_etype};
    end

    if (ipv4_valid) begin
      pkt_hdr_out[159:0] = {ipv4_version, ipv4_ihl, ipv4_diffserv, ipv4_totalLen, ipv4_id, ipv4_flags, ipv4_fragOffset, ipv4_ttl, ipv4_protocol, ipv4_hdrChecksum, ipv4_srcAddr, ipv4_dstAddr};
    end

    pkt_hdr_len = (eth_valid ? 16'd112 : 16'd0) + (ipv4_valid ? 16'd160 : 16'd0);
  end
