module port_fwd_table #(
  ...
  input  logic [8:0] cp_query_key_ingress_port,
  ...
  assign q_key_concat = {3'd0, cp_query_key_ingress_port};
  logic [3:0] q_addr;
  ...
  logic [8:0] q_pend_key_ingress_port;
  logic q_rd_valid;
  logic [8:0] q_rd_key_ingress_port;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      q_pend_addr  <= q_addr;
      q_pend_key_ingress_port <= cp_query_key_ingress_port;
      q_rd_valid   <= mem_valid[q_addr];
      q_rd_key_ingress_port <= mem_key_ingress_port[q_addr];
  ...
  logic q_match; assign q_match = q_rd_valid && (q_rd_key_ingress_port == q_pend_key_ingress_port);
