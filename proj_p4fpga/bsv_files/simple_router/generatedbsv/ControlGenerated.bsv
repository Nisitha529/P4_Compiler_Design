...
`MATCHTABLE_SIM(5, 36, 2, _forward)
typedef Table#(3, MetadataRequest, ForwardParam, ConnectalTypes::ForwardReqT, ConnectalTypes::ForwardRspT) ForwardTable;
typedef MatchTable#(1, 5, 256, SizeOf#(ConnectalTypes::ForwardReqT), SizeOf#(ConnectalTypes::ForwardRspT)) ForwardMatchTable;
...
`MATCHTABLE_SIM(9, 36, 2, _ipv4_lpm)
typedef Table#(3, MetadataRequest, Ipv4LpmParam, ConnectalTypes::Ipv4LpmReqT, ConnectalTypes::Ipv4LpmRspT) Ipv4LpmTable;
typedef MatchTable#(1, 9, 256, SizeOf#(ConnectalTypes::Ipv4LpmReqT), SizeOf#(ConnectalTypes::Ipv4LpmRspT)) Ipv4LpmMatchTable;
...
interface Ingress;
    ...
    method Action _forward_add_entry(ConnectalTypes::ForwardReqT key, ConnectalTypes::ForwardRspT value);
    method Action _ipv4_lpm_add_entry(ConnectalTypes::Ipv4LpmReqT key, ConnectalTypes::Ipv4LpmRspT value);
...
module mkIngress (Ingress);
    ...
    FIFOF#(MetadataRequest) _forward_req_ff <- mkFIFOF;
    FIFOF#(MetadataRequest) _forward_rsp_ff <- mkFIFOF;
    FIFOF#(MetadataRequest) _ipv4_lpm_req_ff <- mkFIFOF;
    FIFOF#(MetadataRequest) _ipv4_lpm_rsp_ff <- mkFIFOF;
    ...
    ForwardMatchTable _forward_table <- mkMatchTable_Forward("_forward");
    Control::ForwardTable _forward <- mkTable(table_request, table_execute, _forward_table);
    messageM(printType(typeOf(_forward_table)));
    messageM(printType(typeOf(_forward)));
    Ipv4LpmMatchTable _ipv4_lpm_table <- mkMatchTable_Ipv4Lpm("_ipv4_lpm");
    Control::Ipv4LpmTable _ipv4_lpm <- mkTable(table_request, table_execute, _ipv4_lpm_table);
    messageM(printType(typeOf(_ipv4_lpm_table)));
    messageM(printType(typeOf(_ipv4_lpm)));
    mkConnection(toClient(_forward_req_ff, _forward_rsp_ff), _forward.prev_control_state);
    mkConnection(_forward.next_control_state[0], setdmac_action.prev_control_state);
    mkConnection(_forward.next_control_state[1], drop2_action.prev_control_state);
    mkConnection(_forward.next_control_state[2], noAction_action.prev_control_state);
    mkConnection(toClient(_ipv4_lpm_req_ff, _ipv4_lpm_rsp_ff), _ipv4_lpm.prev_control_state);
    mkConnection(_ipv4_lpm.next_control_state[0], setnhop_action.prev_control_state);
    mkConnection(_ipv4_lpm.next_control_state[1], drop1_action.prev_control_state);
    mkConnection(_ipv4_lpm.next_control_state[2], noAction_action.prev_control_state);
    ...
    endrule
    rule rl__ipv4_lpm if (_ipv4_lpm_rsp_ff.notEmpty);
        _ipv4_lpm_rsp_ff.deq;
        let _rsp = _ipv4_lpm_rsp_ff.first;
        let meta = _rsp.meta;
        let pkt = _rsp.pkt;
        case (_rsp) matches
            default: begin
        ...
        endcase
    endrule
    rule rl__forward if (_forward_rsp_ff.notEmpty);
        _forward_rsp_ff.deq;
        let _rsp = _forward_rsp_ff.first;
        let meta = _rsp.meta;
        let pkt = _rsp.pkt;
        case (_rsp) matches
            default: begin
    ...
    method _forward_add_entry = _forward.add_entry;
    method _ipv4_lpm_add_entry = _ipv4_lpm.add_entry;
    method Action set_verbosity (int verbosity);
        cf_verbosity <= verbosity;
        _forward.set_verbosity(verbosity);
        _ipv4_lpm.set_verbosity(verbosity);
    endmethod
endmodule
typedef enum {
    REWRITEMAC,
...
`MATCHTABLE_SIM(23, 9, 2, _send_frame)
typedef Table#(3, MetadataRequest, SendFrameParam, ConnectalTypes::SendFrameReqT, ConnectalTypes::SendFrameRspT) SendFrameTable;
typedef MatchTable#(1, 23, 256, SizeOf#(ConnectalTypes::SendFrameReqT), SizeOf#(ConnectalTypes::SendFrameRspT)) SendFrameMatchTable;
...
interface Egress;
    interface PipeIn#(MetadataRequest) prev;
    interface PipeOut#(MetadataRequest) next;
    method Action _send_frame_add_entry(ConnectalTypes::SendFrameReqT key, ConnectalTypes::SendFrameRspT value);
    method Action set_verbosity(int verbosity);
endinterface
module mkEgress (Egress);
    ...
    FIFOF#(MetadataRequest) entry_req_ff <- mkFIFOF;
    FIFOF#(MetadataRequest) entry_rsp_ff <- mkFIFOF;
    FIFOF#(MetadataRequest) _send_frame_req_ff <- mkFIFOF;
    FIFOF#(MetadataRequest) _send_frame_rsp_ff <- mkFIFOF;
    FIFOF#(MetadataRequest) exit_req_ff <- mkFIFOF;
    FIFOF#(MetadataRequest) exit_rsp_ff <- mkFIFOF;
    Control::NoActionAction noAction_action <- mkEngine(toList(vec(step_1)));
    ...
    SendFrameMatchTable _send_frame_table <- mkMatchTable_SendFrame("_send_frame");
    Control::SendFrameTable _send_frame <- mkTable(table_request, table_execute, _send_frame_table);
    messageM(printType(typeOf(_send_frame_table)));
    messageM(printType(typeOf(_send_frame)));
    mkConnection(toClient(_send_frame_req_ff, _send_frame_rsp_ff), _send_frame.prev_control_state);
    mkConnection(_send_frame.next_control_state[0], rewritemac_action.prev_control_state);
    mkConnection(_send_frame.next_control_state[1], drop3_action.prev_control_state);
    mkConnection(_send_frame.next_control_state[2], noAction_action.prev_control_state);
    rule rl_entry if (entry_req_ff.notEmpty);
        entry_req_ff.deq;
        let _req = entry_req_ff.first;
        let meta = _req.meta;
    ...
    endrule
    rule rl__send_frame if (_send_frame_rsp_ff.notEmpty);
        _send_frame_rsp_ff.deq;
        let _rsp = _send_frame_rsp_ff.first;
        let meta = _rsp.meta;
        let pkt = _rsp.pkt;
        case (_rsp) matches
            default: begin
    ...
    interface prev = toPipeIn(entry_req_ff);
    interface next = toPipeOut(exit_req_ff);
    method _send_frame_add_entry = _send_frame.add_entry;
    method Action set_verbosity (int verbosity);
        cf_verbosity <= verbosity;
        _send_frame.set_verbosity(verbosity);
    endmethod
endmodule
